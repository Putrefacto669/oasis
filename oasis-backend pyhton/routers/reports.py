...
# ✅ Crear el router (esto es lo que te faltaba)
...
async def generate_pdf_report():
    ...
    # Tabla de reservas
    story.append(Paragraph("Detalle de Reservas", styles['Heading2']))
    story.append(Spacer(1, 10))
    
    if reservas.data:
        table_data = [['Huésped', 'Habitación', 'Check-in', 'Check-out', 'Total']]
        for r in reservas.data[:20]:
            table_data.append([
                r.get('guest_name', '')[:20],
                r.get('room_name', '')[:15],
                r.get('check_in', ''),
        ...
        table_reservas = Table(table_data, colWidths=[100, 80, 70, 70, 60])
        table_reservas.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#c47a5c')),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
            ('FONTSIZE', (0, 0), (-1, 0), 10),
        ...
        story.append(table_reservas)
    
    # Generar PDF
    doc.build(story)
    pdf_buffer.seek(0)
    
    # ✅ Devolver el PDF correctamente como StreamingResponse
    return StreamingResponse(
        pdf_buffer,
        media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=reporte_oasis_{datetime.now().strftime('%Y%m%d')}.pdf"}
    )
# ============================================
...
async def get_predictive_analysis():
    ...
    x = list(range(len(ocupacion_por_dia)))
